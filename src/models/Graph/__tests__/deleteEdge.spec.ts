...
import {
    ...
    generateNamedVertices, generateUndirectedEdgesWithNameVertex
...
const expectInitialStateUndirected = () => {
    ...
    for(const vertexA of generateNamedVertices(vertices)) {
        for (const vertexB of generateNamedVertices(vertices)) {
            ...
            if (edges.filter(x => (x[0] === vertexA.name && x[1] === vertexB.name) || (x[1] === vertexA.name && x[0] === vertexB.name)).length === 0) {
                expect(graph.getAdjacencyMatrix().get(vertexA, vertexB)).toBe(false);
            } else {
                expect(graph.getAdjacencyMatrix().get(vertexA, vertexB)).toBe(true);
            }
...
const getNonExistentUndirectedEdges = () => {
    ...
    for(const vertexA of vertices) {
        for(const vertexB of vertices) {
            if(edges.filter(x => (x[0] === vertexA && x[1] === vertexB) || (x[1] === vertexA && x[0] === vertexB)).length > 0) continue;
...
describe('graph.deleteVertex(v)', () => {
    ...
    describe('directed edges', () => {
        ...
        describe('edge list is correct', () => {
            it.each(directedEdges)('removes the edge $vertexA.name -> $vertexB.name and receives the correct edge count', (removedEdge) => {
                graph.deleteEdge(removedEdge);
                expect(graph.getListOfEdges().length).toBe(2);
                expectEdgeListsAreEqual(graph.getListOfEdges(), generateDirectedEdgesWithNameVertex(edges).filter(x => !x.equals(removedEdge)));
            });
        });

        describe('adjacencyList is correct', () => {
            it.each(directedEdges)('removes the edge $vertexA.name -> $vertexB.name and removes the entry of $vertexB.name in the list of $vertexA.name', (removedEdge) => {
                graph.deleteEdge(removedEdge);
                expect(graph.getAdjacencyList().getAdjacentVertices(removedEdge.vertexA).filter(x => x.equals(removedEdge.vertexB)).length).toBe(0);
            });

            it.each(directedEdges)('removes the edge $vertexA.name -> $vertexB.name and removes the entry of $vertexA.name in the list of $vertexB.name', (removedEdge) => {
                graph.deleteEdge(removedEdge);
                expect(graph.getAdjacencyList().getAdjacentVertices(removedEdge.vertexB).filter(x => x.equals(removedEdge.vertexA)).length).toBe(0);
            });

            it.each(directedEdges)('removes the edge $vertexA.name -> $vertexB.name and contains other vertices still in the list of $vertexA.name', (removedEdge) => {
                graph.deleteEdge(removedEdge);
                // Works because every node contained in an edge is adjacent to two nodes
                expect(graph.getAdjacencyList().getAdjacentVertices(removedEdge.vertexA).filter(x => !x.equals(removedEdge.vertexB)).length).toBe(1);
            });

            it.each(directedEdges)('removes the edge $vertexA.name -> $vertexB.name and contains other vertices still in the list of $vertexB.name', (removedEdge) => {
                graph.deleteEdge(removedEdge);
                // Works because every node contained in an edge is adjacent to two nodes
                expect(graph.getAdjacencyList().getAdjacentVertices(removedEdge.vertexB).filter(x => !x.equals(removedEdge.vertexA)).length).toBe(1);
            });
        });

        describe('adjacencyMatrix is correct', () => {
            it.each(directedEdges)('removes the edge $vertexA.name -> $vertexB.name and sets the deleted edge to false', (removedEdge) => {
                graph.deleteEdge(removedEdge);
                expect(graph.getAdjacencyMatrix().get(removedEdge.vertexA, removedEdge.vertexB)).toBe(false);
            });

            it.each(directedEdges)('removes the edge $vertexA.name -> $vertexB.name and does not change the other edges to false', (removedEdge) => {
                graph.deleteEdge(removedEdge);
            ...
            it.each(directedEdges)('removes the edge $vertexA.name -> $vertexB.name and all non-existent edges are still false', (removedEdge) => {
                graph.deleteEdge(removedEdge);
    ...
    describe('undirected edges', () => {
        ...
        describe('edge list is correct', () => {
            it.each(undirectedEdges)('removes the edge $vertexA.name <-> $vertexB.name and receives the correct edge count', (removedEdge) => {
                graph.deleteEdge(removedEdge);
                expect(graph.getListOfEdges().length).toBe(2);
                expectEdgeListsAreEqual(graph.getListOfEdges(), generateUndirectedEdgesWithNameVertex(edges).filter(x => !x.equals(removedEdge)));
            });

            it.each(undirectedEdges)('removes the transposed edge $vertexB.name <-> $vertexA.name and receives the correct edge count', (removedEdge) => {
        ...
        describe('adjacencyList is correct', () => {
            it.each(undirectedEdges)('removes the edge $vertexA.name <-> $vertexB.name and removes the entry of $vertexB.name in the list of $vertexA.name', (removedEdge) => {
                graph.deleteEdge(removedEdge);
                expect(graph.getAdjacencyList().getAdjacentVertices(removedEdge.vertexA).filter(x => x.equals(removedEdge.vertexB)).length).toBe(0);
            });

            it.each(undirectedEdges)('removes the edge $vertexA.name <-> $vertexB.name and removes the entry of $vertexA.name in the list of $vertexB.name', (removedEdge) => {
                graph.deleteEdge(removedEdge);
                expect(graph.getAdjacencyList().getAdjacentVertices(removedEdge.vertexB).filter(x => x.equals(removedEdge.vertexA)).length).toBe(0);
            });

            it.each(undirectedEdges)('removes the edge $vertexA.name <-> $vertexB.name and contains other vertices still in the list of $vertexA.name', (removedEdge) => {
                graph.deleteEdge(removedEdge);
                // Works because every node contained in an edge is adjacent to two nodes
                expect(graph.getAdjacencyList().getAdjacentVertices(removedEdge.vertexA).filter(x => !x.equals(removedEdge.vertexB)).length).toBe(1);
            });

            it.each(undirectedEdges)('removes the edge $vertexA.name <-> $vertexB.name and contains other vertices still in the list of $vertexB.name', (removedEdge) => {
                graph.deleteEdge(removedEdge);
                // Works because every node contained in an edge is adjacent to two nodes
                expect(graph.getAdjacencyList().getAdjacentVertices(removedEdge.vertexB).filter(x => !x.equals(removedEdge.vertexA)).length).toBe(1);
            });
        ...
        describe('adjacencyMatrix is correct', () => {
            it.each(undirectedEdges)('removes the edge $vertexA.name <-> $vertexB.name and sets the deleted edge to false', (removedEdge) => {
                graph.deleteEdge(removedEdge);
                expect(graph.getAdjacencyMatrix().get(removedEdge.vertexA, removedEdge.vertexB)).toBe(false);
                expect(graph.getAdjacencyMatrix().get(removedEdge.vertexB, removedEdge.vertexA)).toBe(false);
            });

            it.each(undirectedEdges)('removes the edge $vertexA.name <-> $vertexB.name and does not change the other edges to false', (removedEdge) => {
                graph.deleteEdge(removedEdge);
            ...
            it.each(undirectedEdges)('removes the edge $vertexA.name <-> $vertexB.name and all non-existent edges are still false', (removedEdge) => {
                graph.deleteEdge(removedEdge);
